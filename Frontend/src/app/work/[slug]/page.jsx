...
export default function WorkDetailPage({ params }) {
  ...
  const safe = useMemo(() => {
    ...
    const clean = (html) => DOMPurify.sanitize(normalizeContentForDisplay(html), {
      ADD_TAGS: ['iframe', 'video', 'audio', 'source'],
      ADD_ATTR: ['allow', 'allowfullscreen', 'frameborder', 'controls', 'target'],
    });
    return {
      description: clean(project.description),
      challenges: clean(project.challenges),
      solution: clean(project.solution),
      results: clean(renderResults(project.results)),
      conclusion: clean(project.conclusion),
    };
  }, [project]);
